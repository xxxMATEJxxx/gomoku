...
class Player:
  def __init__(self, player_sign, extrahelping = 0.5, board = None, future = 3):
    self.future = future
  ...
  def play(self, opponent_move):
    # ~ print(f"  {self.future*'  '}timotej{self.sign}: ready to play")
  ...
  def think(self):
    options_number = 2**self.future
    # ~ print(f"  {self.future*'  '}timotej{self.sign}: I am thinking")
    ...
    for x in range(15):
      for y in range(15):
        if self.board.scheme[y][x] == 0:
          ...
          elif cond or (len(options) <= options_number):
            done = False
    ...
    # ~ for i in options:
      # ~ print(f"    {self.future*'  '}{i[0].points} {i[1]}")
    if self.future == 0:
      best = [options[-1][0].points[self.prindex], options[-1][1]]
    else:
      best = [options[-1][0].points[self.prindex], options[-1][1]]
      for opt in options:
        # ~ print(f"  {self.future*'  '}timotej{self.sign}: think about {opt[1]}!")
    ...
    # ~ print(f"  {self.future*'  '}timotej{self.sign}: I think the best is {best[1]} with {best[0]} points")
    return(best)                                      #best = [points, (x, y)]
  def real_price(self, testboard):
    mindplayer = Mindplayer(self.op_sign, self.extrahelping, testboard, self.future-1)
...
class Mindplayer(Player):
  def __init__(self, player_sign, extrahelping, board, future):
